...
def test_make_a_piece_from_an_idea_and_promote_it(client: TestClient, session: Session, brand):
    ...
    response = client.post(f"/ideas/{idea.id}/pieces",
                           data={"type_id": type_id(session, "carousel"), "due_on": "2026-10-01"})
    assert response.status_code == 200

    piece = crud.list_pieces(session, idea_id=idea.id)[0]
    assert piece.title == idea.title
    assert piece.type.name == "carousel"
...
def test_one_idea_can_produce_several_pieces(client: TestClient, session: Session, brand):
    ...
    for piece_type in ("blog", "linkedin post", "x post"):
        client.post(f"/ideas/{idea.id}/pieces", data={"type_id": type_id(session, piece_type)})

    assert {p.type.name for p in crud.list_pieces(session, idea_id=idea.id)} == {
...
def test_filters_by_stage_and_type(client: TestClient, session: Session, brand):
    crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "blog"), title="A blog", stage="draft")
    crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "carousel"), title="A carousel", stage="ready")

    by_stage = client.get("/pieces/list", params={"stage": "ready"}).text
    assert "A carousel" in by_stage and "A blog" not in by_stage

    by_type = client.get("/pieces/list", params={"type_id": type_id(session, "blog")}).text
    assert "A blog" in by_type and "A carousel" not in by_type


def test_due_soonest_first_and_undated_last(session: Session, brand):
    later = crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "blog"), title="Later",
                              due_on=date.today() + timedelta(days=5))
    undated = crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "blog"), title="Undated")
    sooner = crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "blog"), title="Sooner",
                               due_on=date.today())
...
def test_due_pieces_ignores_published_work(session: Session, brand):
    crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "blog"), title="Due today",
                      due_on=date.today())
    crud.create_piece(session, brand_id=brand.id, type_id=type_id(session, "blog"), title="Already out",
...
def test_deleting_a_piece_leaves_the_idea(client: TestClient, session: Session, brand):
    ...
    client.post(f"/ideas/{idea.id}/pieces", data={"type_id": type_id(session, "blog")})
    piece = crud.list_pieces(session, idea_id=idea.id)[0]
